package kz.halyk.maqsat.ai.services.impl;
...
import kz.halyk.maqsat.ai.dto.req.CategorySpend;
import kz.halyk.maqsat.ai.dto.req.ChatMessage;
...
import kz.halyk.maqsat.ai.dto.res.PlannedCategory;
import kz.halyk.maqsat.ai.exceptions.AiResponseException;
import kz.halyk.maqsat.ai.services.AiAssistantService;
...
public class AiAssistantServiceImpl implements AiAssistantService {
    ...
    private String extractJson(String raw) {
        if (raw == null) throw new AiResponseException("AI returned empty response");
        int start = raw.indexOf('{');
        int end = raw.lastIndexOf('}');
        if (start < 0 || end < start) throw new AiResponseException("No JSON object found in AI response");
    ...
    private void validatePlan(BudgetPlanResult plan) {
        ...
        for (PlannedCategory c : plan.categories()) {
            if (c.name() == null || c.name().isBlank()) throw new AiResponseException("Category with blank name");
            if (c.type() == null || !VALID_TYPES.contains(c.type())) throw new AiResponseException("Invalid category type: " + c.type());
            if (c.limitAmount() == null || c.limitAmount().signum() < 0) throw new AiResponseException("Invalid limitAmount for " + c.name());
        }
    }
