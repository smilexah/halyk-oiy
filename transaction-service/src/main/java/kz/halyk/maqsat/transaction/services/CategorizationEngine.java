...
public class CategorizationEngine {
    ...
    private String byMerchant(String merchant) {
        ...
        if (containsAny(m, "magnum", "small", "galmart", "продукт", "market", "супермаркет")) return "Продукты";
        if (containsAny(m, "yandex.go", "yandex go", "indriver", "такси", "taxi")) return "Такси";
        if (containsAny(m, "kfc", "burger", "coffee", "ресторан", "кафе", "cafe", "bar")) return "Рестораны";
        if (containsAny(m, "bus", "metro", "metropolitan", "автобус", "fuel", "petrol", "азс")) return "Транспорт";
        if (containsAny(m, "kazpost", "kommun", "коммунал", "energo", "water", "gas")) return "Коммуналка";
        if (containsAny(m, "cinema", "kino", "theatre", "steam", "game", "развлеч")) return "Развлечения";
        return null;
    }
