...
import kz.halyk.maqsat.summary.services.impl.SummaryLlmServiceImpl;
...
class SummaryLlmServiceTest {
    ...
    void fallback_whenNoApiKey_returnsHardBuiltRussianText() {
        ...
        var openAiClient = new OpenAiClient(WebClient.builder().baseUrl("http://localhost:9999").build(), props);
        var svc = new SummaryLlmServiceImpl(props, openAiClient,
    ...
    void happyPath_withOpenAi_parsesJsonResponse() throws Exception {
        String openAiJson = """
                ...
                """;
        mockWebServer.enqueue(new MockResponse().setBody(openAiJson).addHeader("Content-Type", "application/json"));
