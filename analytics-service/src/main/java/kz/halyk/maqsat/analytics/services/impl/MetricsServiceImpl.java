...
public class MetricsServiceImpl implements MetricsService {
    ...
    public UserMetrics computeForUser(String userId, String period) {
        ...
        List<TxnFact> txns = txnRepo.findByUserIdAndOccurredAtBetween(userId, range[0], range[1]);

        List<TxnFact> expenses = txns.stream().filter(MetricsServiceImpl::isExpense).collect(Collectors.toList());
        List<TxnFact> incomes = txns.stream().filter(MetricsServiceImpl::isIncome).collect(Collectors.toList());

        BigDecimal totalSpent = expenses.stream().map(TxnFact::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal incomeEstimate = incomes.stream().map(TxnFact::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        if (!expenses.isEmpty()) {
            List<BigDecimal> amounts = expenses.stream().map(TxnFact::getAmount).collect(Collectors.toList());
        ...
        if (incomeEstimate.compareTo(BigDecimal.ZERO) > 0) {
            savingsRate = incomeEstimate.subtract(totalSpent).max(BigDecimal.ZERO)
        ...
        catRepo.saveAll(buildCategoryStats(userId, period, expenses));

        detectAndUpsertRecurring(userId, expenses);

        MetricsComputed event = new MetricsComputed(
                userId, period, incomeEstimate, totalSpent, volatility, savingsRate, metrics.getComputedAt());
    ...
    private static boolean isExpense(TxnFact t) {
        if ("CREDIT".equalsIgnoreCase(t.getDirection())) return false;
    ...
    private static boolean isIncome(TxnFact t) {
        if ("CREDIT".equalsIgnoreCase(t.getDirection())) return true;
        String op = t.getOperationType();
        return "SALARY".equalsIgnoreCase(op) || "TOPUP".equalsIgnoreCase(op);
    }
    ...
    private static Instant[] monthRange(String period) {
        ...
        return new Instant[]{
                ...
                ym.plusMonths(1).atDay(1).atStartOfDay(ZoneOffset.UTC).toInstant()
        };
    }
    ...
    private static BigDecimal computeVolatility(List<BigDecimal> amounts) {
        ...
        double vol = Math.sqrt(variance.doubleValue()) / mean.doubleValue();
        return BigDecimal.valueOf(vol).setScale(4, RoundingMode.HALF_UP);
    }

    private static List<CategoryStat> buildCategoryStats(String userId, String period, List<TxnFact> expenses) {
        ...
        for (Map.Entry<String, List<BigDecimal>> entry : grouped.entrySet()) {
            ...
            stat.setAvgAmount(sum.divide(BigDecimal.valueOf(amounts.size()), 2, RoundingMode.HALF_UP));
            stat.setMedianAmount(median(amounts));
            stats.add(stat);
        }
        return stats;
    }
    ...
    private void detectAndUpsertRecurring(String userId, List<TxnFact> expenses) {
        ...
        for (TxnFact t : expenses) {
            byAmount.computeIfAbsent(Math.round(t.getAmount().doubleValue()), k -> new ArrayList<>()).add(t);
        }
        ...
        for (Map.Entry<Long, List<TxnFact>> entry : byAmount.entrySet()) {
            ...
            BigDecimal avgAmt = group.stream().map(TxnFact::getAmount)
                    .reduce(BigDecimal.ZERO, BigDecimal::add)
                    .divide(BigDecimal.valueOf(group.size()), 2, RoundingMode.HALF_UP);

            int modeDay = group.stream()
            ...
            Instant lastSeen = group.stream().map(TxnFact::getOccurredAt)
